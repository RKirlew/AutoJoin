...
def draft():
    ...
    if banThis is not None:
        print("banning phase over")
        while choose is None:
                choose=pyautogui.locateCenterOnScreen("choose.png",grayscale=False,confidence=0.7)
        if choose:
    
                    #pyautogui.moveTo(choose[0],choose[1])
                    print("Choose banner detected!")
                    while c is None:
                        try:
                            print("C is none")
